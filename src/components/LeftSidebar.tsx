...
const LeftSidebar = ({ activeSection = "home", setActiveSection }: LeftSidebarProps) => {
  ...
  const menuItems = [
    ...
    { label: "CAREER", key: "career", active: activeSection === "career", icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg> },
  ];
            ...
            <button
              ...
              className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${
                item.active 
                  ...
                  : 'hover:bg-[#163C0F] hover:text-white'
              }`}
            >
        ...
        <div className="space-y-2">
          {sections.map((section, index) => (
            <div key={index} className="px-3 py-2.5 rounded-md bg-white border border-[#163C0F] hover:bg-[#B3C7AB] transition-colors cursor-pointer">
              ...
              {section.subtitle && (
                <p className="text-[.8 rem] text-[#779E5A] hover:text-white mt-1.5 leading-relaxed">{section.subtitle}</p>
              )}
            </div>
          ))}
        </div>
